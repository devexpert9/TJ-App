...
import { ActionSheetController, ModalController, ToastController, LoadingController, AlertController, Events, IonContent, IonSlides } from '@ionic/angular';
 ...
 //  @ViewChild(IonContent) content: IonContent;
	...
	// selected_subcats:any = [];
  	...
  	getProducts(params = null)
  	{
	    ...
	    this.userService.postData(params,'bestSellerProducts').subscribe((result) => {

	      this.stopLoading();
	      this.products 		    = result.products.reverse();
	      this.total_products 	= result.products.length;

	      var records = Math.ceil(result.products.length/this.show_count);
	      if(isNaN(records)){
	        this.total_pages = 1;
	      }
	      else{
